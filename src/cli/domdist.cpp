...
gemmi::Vec3 selectAtoms(cif::Document doc, char *unp, char *chid, int start, int end);
...
int main(int argc, char *argv[]) {
    ...
    if (input == (char *)nullptr || unp == (char *)nullptr || s1 == -1 || s2 == -1 || e1 == -1 || e2 == -1) {
        usage(argv[0]);
    }
    cif::Document doc = cif::read(gemmi::MaybeGzipped(input));
    auto center1 = selectAtoms(doc, unp, &chid, s1, e1);
    auto center2 = selectAtoms(doc, unp, &chid, s2, e2);
...
gemmi::Vec3 selectAtoms(cif::Document doc, char *unp, char *chid, int start, int end) {
    ...
    for (auto row : doc.sole_block().find("_atom_site.",
        ...
        if (row[0] == "ATOM" && row[5] == "UNP" && 
            !strcmp(row[6].c_str(),unp) &&
            atoi(row[7].c_str()) >= start &&
            atoi(row[7].c_str()) <= end) {
                if (*chid == '-') {
                    *chid = row[1].c_str()[0];
                }
                if (*chid == row[1].c_str()[0]) {
                    mean.x += atof(row[2].c_str());
                    mean.y += atof(row[3].c_str());
                    mean.z += atof(row[4].c_str());
                    std::cout << row[7] << " :: ";
                    std::cout << row[2] << " :: " << row[3] << " :: " << row[4] << " ==> ";
